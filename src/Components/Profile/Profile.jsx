...
export default function Profile() {
    ...
    useEffect(() => {
        async function fetchProfilePicture() {
            const response = await axios.get(`http://localhost:3001/profile_picture/${localStorage.getItem('username')}`, {
                headers: {
            ...
            <section className={`container p-5 my-3 ${styles.profile}`}>
                ...
                <div className={`row p-4 ${styles.userProf}`}>
                    <div className="col-md-2 ">
                        {/* <img src={URL.createObjectURL(profile_picture)} className="w-50 h-50 rounded-circle" alt=""/> */}
